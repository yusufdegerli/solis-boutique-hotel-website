-- Fix create_booking_safe RPC to handle UUIDs instead of BigInt
-- Run this in Supabase SQL Editor

BEGIN;

-- 1. Drop the existing function signatures
...
-- 2. Re-create the function with UUID support
...
RETURNS TABLE (success boolean, data text, error text) -- Changed 'data' from bigint to text (to support UUID)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
    ...
    v_new_id uuid; -- Changed from bigint to uuid
...
BEGIN
    ...
    IF v_room_count IS NULL THEN
        RETURN QUERY SELECT false, NULL::text, 'Oda bulunamadı'::text;
        RETURN;
    END IF;
    ...
    IF v_booking_count >= v_room_count THEN
        RETURN QUERY SELECT false, NULL::text, 'Seçilen tarihlerde boş oda yok'::text;
        RETURN;
    END IF;

    -- 4. Insert Booking
    -- NOTE: Insert requires the table to have 'id' as UUID/Identity. 
    -- Since we confirmed id is UUID, this will return a UUID.
    ...
    RETURN QUERY SELECT true, v_new_id::text, 'Rezervasyon başarılı'::text;
END;
$$;
