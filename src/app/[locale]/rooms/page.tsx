...
import { getRooms, getHotels } from "@/services/hotelService";
...
export default async function RoomsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [rooms, hotels, t] = await Promise.all([
    getRooms(),
    getHotels(),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {rooms.map((room) => {
            ...
            const bookingUrl = hotel ? `/${locale}/hotels/${hotel.slug}/book` : `/${locale}/reservation`;
              ...
              <div key={room.id} className="bg-white rounded-xl overflow-hidden shadow-xl flex flex-col h-full transform hover:-translate-y-1 transition-transform duration-300">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <div className="flex flex-col mb-4">
                    ...
                    {hotel && (
                      <div className="flex items-center gap-1.5 text-[var(--gold)] font-medium text-sm">
                        <MapPin className="w-3.5 h-3.5" />
                        <span className="font-serif">{hotel.name}</span>
                      </div>
                    )}
                  </div>
